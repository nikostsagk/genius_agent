...
public class ExampleAgent extends AbstractNegotiationParty {
    ...
    private static final int SMALL_DOMAIN_THRESHOLD = 100;
    ...
    private double DELTA_REGULARIZATION = 0.000001;
    private boolean sendingSameOffer = false;
    private boolean isSmallDomain = false;
	...
	public void init(NegotiationInfo info) {
	    ...
	    //        delta = domain.getNumberOfPossibleBids() * DELTA_REGULARIZATION;
	    if (domain.getNumberOfPossibleBids() < SMALL_DOMAIN_THRESHOLD) {
	...
	public Action chooseAction(List<Class<? extends Action>> list) {
	    ...
	    if (lastReceivedOffer != null
		                && myLastOffer != null
		&& ((!isSmallDomain && this.utilitySpace.getUtility(lastReceivedOffer) * opponentsAdditiveUtilitySpace.getUtility(lastReceivedOffer) > this.utilitySpace.getUtility(myLastOffer) * opponentsAdditiveUtilitySpace.getUtility(myLastOffer))
		    || (isSmallDomain && this.getUtility(lastReceivedOffer) > this.getUtility(myLastOffer))) ) {
		...
		myLastOffer = sendingSameOffer || isSmallDomain?  generateRandomBidWithUtility(utility) : generateBestBidWithinRangeForOpponent(utility);
		return new Offer(this.getPartyId(), myLastOffer);
